...
| `RunsPage` | Runs list: page header with relative last-read, Refresh, and Admin-only Run pipeline. Status filter chips, then rows in a hairline panel sharing one grid so run and step badges align; a row expands in place. Load more keeps the pages it has. Explicit empty, filtered-empty, loading, and pipeline-unreachable states. |
